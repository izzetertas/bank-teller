...
- Two transaction types: **deposit** and **withdrawal**, chosen with a segmented
  ...
  balance is zero. The form resets to deposit mode, with the amount and any
  error cleared, whenever the teller switches to a different account.
...
- On submit, the amount is parsed and must satisfy all of the following, each with
  ...
  - at most $1,000,000,000.00 — "Amount exceeds the maximum of $1,000,000,000.00",
    with the maximum worded in the account's currency
...
host. There is no Node server to run — `next start` refuses to work with a
static export — so `npm run start` serves `out/` with `serve` instead. The e2e
suite serves this exact artifact, so what gets tested is what gets shipped.
