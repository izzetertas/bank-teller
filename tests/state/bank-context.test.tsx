...
describe('useBank', () => {
  ...
  it('creates, selects, and transacts through its own service', () => {
    ...
    let deposit: ReturnType<typeof result.current.deposit> = {
      ok: false,
      error: '',
    };
    act(() => {
    ...
    expect(
      result.current.selectedAccount?.transactions[0]?.timestamp,
    ).toBeGreaterThan(0);
  });
  ...
  it('surfaces the service error when a rejected mutation reaches it', () => {
    ...
    let created: ReturnType<typeof result.current.createAccount> = {
      ok: true,
      accountId: '',
    };
    act(() => {
