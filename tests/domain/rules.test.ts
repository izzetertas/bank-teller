...
describe('validateAmount', () => {
  ...
  it('rejects an amount above the per-transaction maximum, worded in the account currency', () => {
    expect(validateAmount(accountWithBalance, MAX_TRANSACTION_CENTS)).toEqual({
      ok: true,
    });
    expect(
      validateAmount(accountWithBalance, MAX_TRANSACTION_CENTS + 1),
    ).toEqual({
      ok: false,
    ...
    expect(
      validateAmount(
        { ...accountWithBalance, currency: 'EUR' },
  ...
  it('does not look at the balance', () => {
    expect(
      validateAmount({ ...accountWithBalance, balanceCents: 0 }, 100_000),
    ).toEqual({
      ok: true,
    });
  });
...
describe('validateWithdrawal', () => {
  ...
  it('reports an overdraft with the current balance in the account currency', () => {
    ...
    expect(
      validateWithdrawal({ ...accountWithBalance, currency: 'EUR' }, 501),
    ).toEqual({
      ok: false,
      error: 'Insufficient funds — the balance is €5.00',
    });
